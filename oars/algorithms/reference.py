...
def pg_extra(data, A, B, W, bar_W=None, Ls=1., itrs=1001, alpha=1.0, warmstartprimal=None, verbose=False, callback=None):
    ...
    Args:
        ...
        B (list): list of grad operator classes with Lipschitz parameter <= 1 (or alpha scaled appropriately)
        W (ndarray): mixing matrix (n, n)
        bar_W (ndarray): another mixing matrix (n, n), often (W + I)/2
        Ls (float, optional): max Lipschitz constant of the gradient functions for checking alpha, default 1
    ...
    if verbose:
        print('date\t\ttime\t\titr\t||x-bar(x)||\t||sum (sub)gradients||')
        checkperiod = max(itrs // 10, 1)
    ...
    for itr in range(itrs//2):
        ...
        if verbose and itr*2 % checkperiod == 0:
            ...
            subgradient_norm = np.linalg.norm(np.sum([half_x[i] - x_one[i] + grad_zero[i] for i in range(n)], axis=0))
            print(f"{datetime.now()}\t{itr*2}\t{consensus_error:.3e}\t{subgradient_norm:.3e}")
