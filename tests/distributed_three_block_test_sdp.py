import numpy as np
from oars.algorithms import distributed_three_block_solve, distributed_block_solve
...
proxlist = [nlinearSubdiff, ntraceEqualityIndicator, ntraceEqualityIndicator] + [ntraceHalfspaceIndicator for _ in Kp]
pos = 1
...
data = [-Ko, {'A':Ki, 'v':1}, {'A':K1, 'v':0}] + Kp
data.insert(pos, xdim)
dim = len(data)
warmstart = np.zeros(xdim) #sum(Kp + [Ko, -K1/(2*tgt_n), Ki/(tgt_n-1)])
x, results = distributed_three_block_solve(dim, data, proxlist, warmstartprimal=warmstart, itrs=5001, gamma=0.99, vartol=1e-5, verbose=True)
# Fiedler 0.157, 0.243 
# Similar 0, 4/dim (0.4)
